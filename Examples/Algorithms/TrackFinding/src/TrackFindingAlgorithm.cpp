...
ProcessCode TrackFindingAlgorithm::execute(const AlgorithmContext& ctx) const {
  ...
  Acts::GeometryIdentifier geoId(72058418671649024);
  //geoId.setVolume(1);
  //geoId.setLayer(12);
  //geoId.setApproach(0);
  auto pSurface = m_cfg.trackingGeometry->findSurface(geoId);
  //auto pSurface = Acts::Surface::makeShared<Acts::Surface>();
