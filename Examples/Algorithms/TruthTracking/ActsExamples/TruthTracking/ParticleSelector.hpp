...
class ParticleSelector final : public IAlgorithm {
 public:
  ...
  struct Config {
    ...
    // Measurement number cuts
    std::size_t measurementsVSMin = 0;
    std::size_t measurementsVSMax = std::numeric_limits<std::size_t>::max();
    // Measurement number cuts
    std::size_t measurementsMSMin = 0;
    std::size_t measurementsMSMax = std::numeric_limits<std::size_t>::max();
