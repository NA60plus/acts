...
#include "MatchingPerformanceWriter.hpp"
...
ActsExamples::MatchingPerformanceWriter::MatchingPerformanceWriter(
    ActsExamples::MatchingPerformanceWriter::Config config,
    Acts::Logging::Level level)
    : WriterT(config.inputTracks, "MatchingPerformanceWriter", level),
      m_cfg(std::move(config)),
      m_matchingPlotTool(m_cfg.matchingPlotToolConfig, level) {
  ...
  m_matchingPlotTool.book(m_matchingPlotCache);
}

ActsExamples::MatchingPerformanceWriter::~MatchingPerformanceWriter() {
  m_matchingPlotTool.clear(m_matchingPlotCache);
  if (m_outputFile != nullptr) {
    m_outputFile->Close();
  }
}

ActsExamples::ProcessCode ActsExamples::MatchingPerformanceWriter::finalize() {
  if (m_outputFile != nullptr) {
    m_outputFile->cd();
    m_matchingPlotTool.write(m_matchingPlotCache);
    ACTS_INFO("Wrote performance plots to '" << m_outputFile->GetPath() << "'");
  }
  return ProcessCode::SUCCESS;
}

ActsExamples::ProcessCode ActsExamples::MatchingPerformanceWriter::writeT(
  ...
  ev_counter += 1;
  m_matchingPlotTool.fill(m_matchingPlotCache, recPrimaryVertex, genPrimaryVertex); 
  if (m_outputFile != nullptr) {
